'use client';
import { ArrowLeft, BackpackIcon, MoveLeft } from "lucide-react";
...
export default function SingleUser({ params }: { params: Promise<{ id: string }> }) {
	...
	return (<>
		...
		<Link href={'/dashboard/users'} className="cursor-pointer inline-flex align-middle items-center gap-2 group relative  py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
		...
		<div className="max-w-md w-full space-y-8 mx-auto">
			...
			<div className="bg-white py-8 px-6 shadow rounded-lg sm:px-10">
				<form className="space-y-6" onSubmit={handleUpdate}>
					...
					<div>
						<button type="submit" className="cursor-pointer group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Update</button>
					</div>
				</form>
			</div>
		</div>
